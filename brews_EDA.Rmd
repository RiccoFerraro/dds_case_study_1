...
```
# 7. Summary statistics and Histogram for ABV
```{r}
...
bdat.imputed.IBU.clean %>% ggplot(aes(x=ABV, y=log(IBU.clean), colour=State)) + geom_point()+ ggtitle("log(IBU) vs ABV")
bdat.imputed.IBU.clean %>% ggplot(aes(x=ABV, y=log(IBU.clean))) + geom_smooth()+ ggtitle("log(IBU) vs ABV")

bdat.imputed.IBU.clean %>% ggplot(aes(x=log(ABV), y=log(IBU.clean), colour=State)) + geom_point()+ ggtitle("log(IBU) vs log(ABV)")
bdat.imputed.IBU.clean %>% ggplot(aes(x=log(ABV), y=log(IBU.clean))) + geom_smooth()+ ggtitle("log(IBU) vs log(ABV)")
...
#Log-Linear
fullySaturatedModel_loglinear <- lm(log(IBU.clean)~State + State*ABV + ABV, data=bdat.imputed.IBU.clean)
plot(loglinear)

#Log-Log
loglog <- lm(log(IBU.clean)~State + State*log(ABV) + log(ABV), data=bdat.imputed.IBU.clean)
plot(loglog)

#Linear Linear is the winner winner? 
```
