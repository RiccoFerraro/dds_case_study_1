...
bdat.IPA.Vs.Ales %>% ggplot(aes(x=ABV, fill= Style)) + geom_histogram() + ggtitle("IBU vs ABV for IPA's and OtherAles")
bdat.IPA.Vs.Ales %>% ggplot(aes(x=ABV, y=IBU.clean, colour= Style)) + geom_point() + geom_smooth(method="lm") + ggtitle("Histogram of IPA vs OtherAle for ABV")
...
#Conclusion
There are several note-worthy relationships between in our craft beer dataset. For example, median IBU as well as median ABV per state vary by state for craft beers vary. When targeting sales of particular states' craft beer, it would be wise to consider the median IBU and median ABV for the current craft beers in that state. 
