---
title: "brewsTest"
authors: "Kebur Fantahun", "Ricco Ferraro"
date: "02/21/2021"
output:
...
# 8a. Data Prep for Difference with respect to IBU and ABV between IPAs and other Ales
```{r}
...
classifications <- predict(fit, bdat.IPA.Vs.Ales.test)
...
Todo: make the unit increase in interpretation be in terms of .01 unit increase in ABV
```{r}
...
# 8.e LDA and KNN to predict style based on IBU and ABV
```{r}
bdat.IPA.Vs.Ales.train
...
lda.fit.IBU = train(Ounces ~ IBU.clean, data=bdat.IPA.Vs.Ales.train.Extra, method="qda",
                trControl = trainControl(method = "cv"),
                             metric     = "Accuracy")
lda.fit.IBU
...
knn.fit.IBU <- train(Style ~ IBU.clean,
             ...
             trControl  = trControl,
             metric     = "Accuracy",
             data       = bdat.IPA.Vs.Ales.train.Extra)
knn.fit.IBU

print("use LDA!")
# todo: address assumptions for LDA
#Add a chart (states vs IPA count? or something like that)
```

# 9.b Linear Regression for Ounces vs 

Todo: make the unit increase in interpretation be in terms of .01 unit increase in ABV
```{r}
linearlinear <- lm(IBU.clean~Style +Style*ABV + ABV, data=ç)
plot(linearlinear)
confint(linearlinear)
summary(linearlinear)
```
